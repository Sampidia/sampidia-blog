...
export function parseMarkdownToHtml(markdown: string): string {
  ...
  cleanText = cleanText.replace(/^###\s*(.*?)$/gm, '<h3 class="text-lg font-bold mt-6 mb-2 text-slate-900 dark:text-slate-100 font-heading">$1</h3>');
  ...
  const formatted = paragraphs.map(p => {
    ...
    return `<p class="mb-4 text-slate-800 dark:text-slate-200 leading-relaxed text-sm md:text-base">${trimmed}</p>`;
  });
